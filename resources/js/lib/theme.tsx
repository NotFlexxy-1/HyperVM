...
const ThemeContext = createContext<{ mode: Mode; setMode: (m: Mode) => void; resolved: 'light' | 'dark' }>({
    mode: 'dark',
    setMode: () => undefined,
    resolved: 'dark',
});
...
export function ThemeProvider({ children }: { children: ReactNode }) {
    ...
    const theme = page.props.settings?.theme;
    const stored = typeof window !== 'undefined' ? (localStorage.getItem('hv-mode') as Mode | null) : null;
    const [mode, setModeState] = useState<Mode>(stored ?? theme?.default_mode ?? 'dark');
    ...
    // Live-apply admin colour changes without a reload.
    useEffect(() => {
        if (!theme) return;
